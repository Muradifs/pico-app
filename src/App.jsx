  ...
  const formatTime = (dateVal) => {
    ...
    try {
      if (dateVal && typeof dateVal.toDate === 'function') {
        return dateVal.toDate().toLocaleTimeString();
      }
...
const PicoApp = ({ isNodeRunning }) => {
  ...
  // --- FIREBASE AUTH & SYNC ---
  useEffect(() => {
    // 1. Auth Init (with Error Handling for Offline Mode)
    const initAuth = async () => {
      try {
        ...
        console.warn("Auth Init Failed (Offline Mode Active):", err);
      }
    };
    initAuth();
  ...
  useEffect(() => {
    ...
    // Using sanitized appId for safe path
    const userDocRef = doc(db, 'artifacts', appId, 'users', firebaseUser.uid, 'data', 'profile');
    
    // Wrap snapshot in try/catch to prevent crashes if permission denied
    ...
    try {
        unsubscribeSnapshot = onSnapshot(userDocRef, (docSnap) => {
          if (docSnap.exists()) {
            ...
            console.warn("Firestore sync error (likely permission/offline):", error);
        });
    } catch (e) {
  ...
  const connectWallet = async () => {
    // Try auth one more time if missing, but don't block
    if(!firebaseUser) {
        try {
            await signInAnonymously(auth);
        } catch (e) {
    ...
    setTimeout(async () => {
        ...
        // Try to sync to DB if user exists, otherwise just local state
        if (auth.currentUser) {
            const userDocRef = doc(db, 'artifacts', appId, 'users', auth.currentUser.uid, 'data', 'profile');
            try {
                ...
                console.warn("Offline/Permission error, using local state");
                setUserData(prev => ({ ...prev, ...newUserData }));
                triggerNotification("Welcome (Offline): +1.0 PiCo");
            }
        } else {
            // Offline mode
            setUserData(prev => ({ ...prev, ...newUserData }));
  ...
  const updateRemoteState = async (updates) => {
      if(!auth.currentUser) return; // Skip if offline
      try {
        const userDocRef = doc(db, 'artifacts', appId, 'users', auth.currentUser.uid, 'data', 'profile');
        await updateDoc(userDocRef, updates);
      } catch (e) {
        console.warn("Sync failed (offline/local mode)");
      }
  };
  ...
  const startKyc = async () => {
      ...
      setTimeout(async () => {
          ...
          setUserData(prev => ({
              ...prev, 
              kycStatus: 'verified',
              balance: prev.balance + KYC_BONUS
          }));
